...
最新实验见 [纯译文、局部修正与恢复验证](staged-closure-20260913.md)：整章纯文本
翻译及独立提取完成；19 段局部修正后其余 95 段保持不变，七项恢复检查通过。
最后核验仍产生新补丁并反转先前意见，却声明通过，完整自动接受未通过。
证据检查与内容验收分别记录，正式后台未接入、未发布。

上一实验见 [整章翻译、独立提取与多模型复核](staged-production-20260912.md)：
...
- [`provider-isolation-20260912-deepseek.json`](provider-isolation-20260912-deepseek.json)：
  绕过 Chronicle 直接调用 CPA 的八个有界参数／流式探针；七次返回完整的
  1..200 数列且报告输出超过请求的 16 token，一次网络请求失败、用量未知。
  Responses 与 Chat 的非流式、流式均复现额度未执行；关闭推理的两种成功
  请求仍报告推理用量。部署版本为 v7.2.151 / 5208aec，但管理接口返回 401，
  尚未取得实际路由配置或转发请求，不能继续区分 CPA 与它的上游。

## 此前 v6 与原 DeepSeek 通道的测试结论（2026-09-12，保留）
...
随后进行的独立 HTTP 诊断将输出参数问题定位到 CPA／上游调用链：Chronicle
没有参与请求，16-token 额度仍未得到执行。Responses 非流式实际报告 652
输出 token，Chat 的 `max_tokens` / `max_completion_tokens` 分别报告 1,104 /
630；流式分别报告 912 / 642。七次成功返回均经原始数据复核，完整数列、
正文及 HTTP SHA／长度的 42 项检查通过；另外 39 项本地 provider／配置接线
单测通过。流式小请求能持续接收数据，但不能据此宣布整章通过或解除单次生成
额度；当前产品 provider 仍为非流式。本次没有新整章生成或产品运输代码变更。
