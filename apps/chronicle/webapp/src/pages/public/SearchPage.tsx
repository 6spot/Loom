...
function MatchedSurface({ surface }: { surface: SearchSurface }) {
  return <li>{surface.source_title ? <span className="match-source">{surface.source_title}</span> : <span className="match-source">canonical 展示</span>} <span className="match-field">{surface.field}</span> <span className="match-kind">{MATCH_LABEL[surface.match ?? ""] ?? surface.match ?? "匹配"}</span> <q>{surface.value}</q></li>;
}

function EntityCard({ item, timeSearch }: { item: SearchItem; timeSearch: string }) {
  return (
