...
范围：第二轮 [#549](https://github.com/6spot/Loom/issues/549) 的界面与交互验收要求。数据含义、路由和发布边界以 [continuous-reading.md](continuous-reading.md) 为准。此文固定布局、交互和可观察结果；task 文档记录任务范围、接口前置和验收要求。

新增背景设计见 [background-art.md](background-art.md)：按时代制作淡彩背景，AI 可建议位置，用户明确触发生成/上传并校验保存后才展示。仅页面背景，不插入正文；滚动和内容生产均不自动生成图片。技能/离线候选库由 D01 提供，产品上传、保存/关联和背景渲染另待拆分。

2026-09-11 用户确认主体正文为“综合多份史料生成新的连续历史叙事，结论可追溯”，按时间正序阅读；事件／时期入口只定位，不筛选后续正文。正式页面左侧放时间轴，中间保持连续正文，右上放随阅读位置更新的附近事件，右下以紧凑行展示人物与地点的当时状态，依据按需深入查看。用户要求直接替换正式前台。已确认要求、人物页职责及相对 R2／R3 的缺口见 [historical-narrative-design.md](historical-narrative-design.md)；下文仍限定 R2 的来源阅读合同，不能以静态样例代替新生产链路或真实验收。
...
若短段落位于正文真实首尾，浏览器滚动边界可能使目标无法对齐参考线。此时仍保留明确请求的段落与人物状态；布局测量不能擅自切到另一段，用户继续滚动后再恢复自动跟读。
...
集成后的 `webapp/scripts/reading-flow-smoke.mjs` 由当前 staged 验收任务提供，接口 `--base-url http://127.0.0.1:18080 --fixture-manifest <path> --suite flow|accessibility|performance|all --output <directory>`；真实 Rust/Python/PG 数据和 manifest 由 `acceptance/staged_gate.py` 准备，不用 HTTP mock 冒充整链。篇章、阅读和人物状态共用 `gate_runtime` 生命周期、隔离 Compose 栈、Studio/API、browser driver 和清理，不能复制另一套产品写入路径。fixture 可使用固定测试决定但只证明程序机制；live 禁用 fixture 并等待人工身份审核，人工内容核对期间保留受控会话。单独 browser 命令只用于仍在运行的栈；UI gate 接入当前 Chronicle workflow，真实 provider 与内容质量另记录，见 `reading-acceptance.md`。
