...
- **`unit_phases` stay translation-bound.** A unit-phase binding's
  ...
  time.
- **Origin evidence and anchor payloads are preserved, never merged.**
  Every item keeps an `origin` block with its chapter, chapter index,
  origin revision/local ref and artifact hash; the per-chapter
  `person_state_evidence` manifest adds the resolved anchor ids **and their
  immutable payload records**. Acceptance unions the person-state anchor
  payloads into the artifact `anchors`, so every cited `anchor_id` resolves
  through the same anchor set the source reader uses; assembly never leaves
  a citation backed only by a bare id. Assembly assigns no canonical ID and
  introduces no same-link: same-name entities or offices stay distinct.
- **Candidate metadata is coverage-checked and hash-bound.** The accepted
  `person_state_candidates` keys are generated by acceptance and bound into
  `artifact_sha256`, and assembly re-validates that there is exactly one
  entry per state item with the allowed kind, an item matching the state
  item's phase/fact refs, a `candidate_key` recomputed from the resolved
  anchors, and only anchors that have payloads. A missing, extra, duplicated
  or inconsistent entry rejects the whole assembly.
...
- **Fail closed.** Missing/extra/duplicate chapters, mixed generations,
  tampered `person_states_sha256`/artifact cores/candidate metadata list,
  duplicate local state IDs, candidate coverage gaps and dangling
  phase/fact/entity/claim/block/anchor references reject the whole assembly
  instead of publishing a partial state set.
