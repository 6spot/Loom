...
后台布局见 [ui.md 的 Studio content workspace](ui.md#studio-content-workspace)。队列以资料名称、审核类型和待核对数量为主，任务筛选使用资料名称与创建时间。章节内容审核按正文与提取信息、待核对意见、模型结果与历史切换，表单保留挂载，切换不丢编辑内容。综合历史正文默认展示连续阅读预览，精选入口与当前人物／地点的已保存阶段状态同步；编辑正文与入口在同页切换。预览不发布，所有决定仍走本指南的固定版本与完整意见校验。
