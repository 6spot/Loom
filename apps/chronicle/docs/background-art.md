...
状态：产品设计、后端素材/关联基础设施和 Studio 候选管理/人工保存界面已实现；读者页面背景组件仍由后续任务负责。[D01/#588](https://github.com/6spot/Loom/issues/588) 交付的可复用技能与离线候选归档工具仍与生产素材库分离。本文件不改变既有古文文本上传接口或阅读产物 schema。
...
1. 阅读背景组件、已保存关联读取、单一阅读 controller 接线与无图回退。
