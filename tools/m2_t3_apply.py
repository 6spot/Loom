...
# Runtime persistence contract: CommitStore becomes executor-neutral async and
# gains an infrastructure failure variant instead of misclassifying SQL errors.
path = Path("crates/loom-runtime/src/persistence.rs")
text = path.read_text()
text = replace_once(
    text,
...
text = replace_once(
    ...
    "CommitStore async signature",
)
path.write_text(text)

# Runtime orchestration awaits CommitStore while WorkStore remains T4-sync.
path = Path("crates/loom-runtime/src/orchestration.rs")
text = path.read_text()
text = replace_once(
    text,
    "        match self.store.commit(&validated, Some(&claim), now) {",
    "        match self.store.commit(&validated, Some(&claim), now).await {",
    "execute_work commit await",
)
text = replace_once(
    text,
    ...
    "reference CommitStore async forwarding",
)
text = replace_once(
    text,
    "                    self.store\n                        .commit(&validated, None, self.platform_clock.now())\n                        .map(|result| {",
    "                    self.store\n                        .commit(&validated, None, self.platform_clock.now())\n                        .await\n                        .map(|result| {",
    "ActionService commit await",
)
text = replace_once(
    text,
    ...
    "        }\n"
    "        CommitError::DuplicateEvent { .. }",
    "commit storage error mapping",
)
path.write_text(text)

# In-memory adapter keeps its synchronous inherent implementation but satisfies
# the Runtime port with an immediately-ready executor-neutral future.
path = Path("crates/loom-storage/src/in_memory.rs")
text = path.read_text()
text = replace_once(
    text,
    "impl CommitStore for InMemoryStore {\n    fn commit(\n        &self,\n        resolution: &ValidatedResolution,\n        current_work: Option<&WorkClaim>,\n        now: PlatformTime,\n    ) -> Result<CommitResult, CommitError> {\n        self.commit(resolution, current_work, now)\n    }\n}",
    "impl CommitStore for InMemoryStore {\n    fn commit<'a>(\n        &'a self,\n        resolution: &'a ValidatedResolution,\n        current_work: Option<&'a WorkClaim>,\n        now: PlatformTime,\n    ) -> PersistenceFuture<'a, Result<CommitResult, CommitError>> {\n        Box::pin(async move { InMemoryStore::commit(self, resolution, current_work, now) })\n    }\n}",
    "InMemory CommitStore async adapter",
)
path.write_text(text)

# Keep the PostgreSQL read module focused; the child commit module can access
# PgStorage's private pool without exposing SQLx outside loom-storage.
path = Path("crates/loom-storage/src/postgres.rs")
text = path.read_text()
text = replace_once(
    text,
    ...
    "mod commit;\n\n",
    "PostgreSQL commit child module",
)
path.write_text(text)

commit = r'''//! PostgreSQL implementation of the Runtime-owned atomic CommitStore port.

use std::collections::HashSet;

use loom_core::{EventId, EventSeq, FacetOwner, StateRevision, TimelineId, TimelineVersion, WorldEffect};
...
async fn lock_timeline(
    ...
    Ok(LockedTimeline {
        ...
        world_time: loom_core::WorldInstant::new(
            row.try_get("world_time").map_err(storage_error)?,
        ),
    })
}
...
async fn apply_event(
    ...
    let effects = serde_json::to_value(&event.effects)
        .map_err(|error| invalid_event(event.id, format!("cannot serialize frozen Effects: {error}")))?;
    sqlx::query(
    ...
    match effect {
        WorldEffect::CreateEntity { entity_id } => {
            if entity_id.is_nil() || entity_exists(transaction, timeline_id, *entity_id).await? {
                return Err(invalid_effect(event_id, "Entity identity is nil or already exists"));
            }
            sqlx::query(
        ...
        WorldEffect::EndRelationship { relationship_id } => {
            ...
            match active {
                ...
                Some(false) => return Err(invalid_effect(event_id, "Relationship is already ended")),
                Some(true) => {}
            }
            sqlx::query(
...
async fn apply_work_mutation(
    ...
    match mutation {
        WorkMutation::Schedule(work) => schedule_work(transaction, timeline_id, work, now).await,
...
async fn schedule_work(
    ...
    let due_world_time = match work.schedule {
        ...
        WorkSchedule::At(instant) => Some(instant.value()),
    };
    sqlx::query(
...
async fn validate_current_work(
    ...
    let actual_fence = fence.parse::<u64>().map_err(|error| {
        CommitError::StorageUnavailable {
            message: format!("invalid persisted Work fence: {error}"),
        }
    })?;
    if actual_fence != claim.fence()
        || PlatformTime::new(claimed_until) != claim.claimed_until()
    {
...
fn parse_u64(row: &sqlx::postgres::PgRow, column: &str) -> Result<u64, CommitError> {
    ...
    value.parse().map_err(|error| CommitError::StorageUnavailable {
...
'''
commit_path = Path("crates/loom-storage/src/postgres/commit.rs")
