...
Each scenario below expands the 10 required matrix columns so T10–T18 Executors can implement without semantic choice. Any row requiring a new authority decision is marked `blocked` and escalated — at freeze 2 rows are blocked (`CV-028`, `CV-029`) for missing public API (see Coverage Gaps 8/9); all others implementable via existing `loom-api`/`loom-client`; future discovery of additional missing authority must also mark blocked and stop.
...
- **Expected Result:** `invoke` returns `Ok(ExecutionResult::Committed { event_ids: Vec<EventId>, timeline_version: TimelineVersion })` where `event_ids.len()==1`. `get_facet` returns `Some(FacetSnapshot { value: {"value":1}, schema_revision })`. `list_events`/.`list_events_page` contains `CommittedEvent { id: EventId, type: "neutral.counter.seeded", occurred_at: pinned world_time, payload }` ordered by `EventSeq` and `timeline_version.head_event_seq` advances by 1.
...
- **Expected Result:** After platform `Retryable(IngressTechnicalFailure)` failure, `list_events` still count before recovery (0 or prior), `get_facet` unchanged. After retry/success path, `IngressStatusRecord.status` becomes `IngressStatus::Completed(IngressCompletion::Committed { event_refs, timeline_version })` with exactly one `EventRef`; no duplicate factual Events even with transport retry; `IngressStatus::Retryable` never rendered as `IngressStatus::Completed(IngressCompletion::Rejected)`.
...
- **Preconditions:** Head Work claimed by worker A with fence generation `g1`; lease expired or `AdminService::terminalize_work(AdminTerminalizeWorkRequest { target, expected_version, reason })` via authorized path moves generation to `g2` held by worker B.
