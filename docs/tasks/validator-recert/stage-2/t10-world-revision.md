...
- `cv014`: creates World under R1, seeds value 7 to establish historical identity, captures `list_events` count, `catalog_for_world` ids, `inspect_timeline` version/world_time, and `active_runtime_revision` generation; lists revisions to find a compatible R2 (`revision_id != R1` and contains `neutral.counter`); if no R2 is published, returns `Unavailable` with explicit `fixture requires compatible R2` (no new authority is invented); activates R2 with `expected_generation`; verifies `list_events` count unchanged and payload unchanged, `catalog_for_world` ids unchanged, `inspect_timeline` version/world_time not rewritten, forks a child and verifies its first `seed` commits under R2 while original history does not leak the fork event, and optionally exercises `BackendContext::restart()` when `ControlledBoundaryRestart` is available to prove durable history/binding survive a real boundary rebuild.
...
- `cv014_revision_activation_preserves_binding_on_real_in_memory_with_r2` — custom `InMemoryR2Server` that confirms both `validator-explicit-v0` (R1) and `validator-t10-r2` (R2, same caps, different build) before serving, then `BackendContext` with `InMemory` + `ControlledBoundaryRestart`; asserts `Pass` and evidence cites `list_runtime_revisions`/`activate_runtime_revision`/`inspect_timeline`/`list_events`/`catalog_for_world`/`invoke`; verifies no storage import.
- `cv014_revision_activation_preserves_binding_on_live_postgres_with_r2_when_configured` — custom `PgR2Server` (same dual-revision logic over `PgStorage` with `migrate`/`health` and repository postgres auto-start); when live postgres is unavailable, the test accepts the validator's correct `Skipped`/`Unavailable` (missing `LOOM_TEST_POSTGRES_URL` or unreachable) rather than synthetic pass; when live, it asserts `Pass` and `backend_evidence:postgresql`.
...
- Dual-revision helpers are defined locally in the test file; `tests/common/mod.rs` is not edited — the extra R2 publication is isolated to this suite's own fixture wiring, satisfying the "no shared helper unless proven" rule while still providing the matrix-required `list_runtime_revisions` fixture.
...
- `cargo test -p loom-validator --test world_binding` → 9 passed (1 InMemory CV-012, 1 Postgres CV-012, 1 InMemory CV-013, 1 Postgres CV-013, 1 InMemory R2 CV-014, 1 Postgres R2 CV-014, 1 external CV-014, 1 descriptor, 1 scaffold); no ignored
