...
- The second-round reading semantics fail on real content: every `year_key` is
  ...
  資治通鑑 chapter has no events and one group; generation needed up to four job
  attempts, failing closed on the `reading_time`/anchor contract rules.
- **Not accepted.** The failures are generation/projection contract defects to be
  fixed by the owning leaves and re-verified here. Independent human content
  confirmation, the live browser walkthrough, and provider cost capture remain
  unverified.
