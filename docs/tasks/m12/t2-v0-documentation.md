...
Rework D-004/D-006/D-007/D-008/D-009/D-010 on head `6605119` → new head (see below):
- D-004 Compose Scheduler passthrough — added `LOOM_SCHEDULER_WORLD_ID`/`LOOM_SCHEDULER_TIMELINE_ID` to `compose.yaml:24-36` (`:-` empty default) and patched `apps/loom-server/src/config.rs:367-380` to treat empty/whitespace as `None` (avoids `missing field template` style empty-string parse failure). Verified `docker compose config` unset → `""` (still None via code) and set → UUIDs render correctly; after World creation `echo LOOM_SCHEDULER... >> .env` + `docker compose up -d --build loom-server` creates worker (vs previous head where unset always None and set never reached server).
- D-006 deterministic fake deferred — removed/rewrote `README.md:212` `Local examples use ... deterministic fake` and `docs/capacity-envelope.md:125` `deterministic fake is the supported V0 example`; now both state deterministic fake is benchmark/test evidence only (`loom-bench`/`loom-agency/src/testing.rs`), default server uses `UnavailableCognitiveExecutor`, and a supported public deterministic fake fixture/adapter is **deferred** to `M12-T3`/T4; `docs/README.md` quickstart link also now notes `WorldTemplateDescriptor`/scheduler-after-target-config and deferred Agency.
- D-007 Admin token audit — fixed `docs/quickstart.md:261` `admin timeline status` to include global `--admin-token $LOOM_ADMIN_TOKEN` (previously only `--output`); swept all `admin` commands in `quickstart.md`/`operator-guide.md`/`README.md` to ensure every `admin` example carries non-empty `--admin-token` before subcommand; no hard-coded secret (via `LOOM_ADMIN_TOKEN` env/flag, `.env.example` has no value).
- D-008 feed resume — replaced top-level `after` JSON with `resume_from: ChangeFeedCursor{target, after}` per `SubscriptionRequest:995-1001`; verified via `validate_sub` that `after` alone is ignored (`resume_from=None`) while correct `resume_from` deserializes `OK`. Fixed `--request-file` example to `cargo run -p loom-cli -- --output human feed subscribe --world $WORLD --timeline $TIMELINE --request-file /tmp/sub.json` (adds required `--world`/`--timeline`; previously missing and returned exit 2). Both `--after/--limit` and `--request-file` forms now parser-validate with `cargo run -p loom-cli --quiet -- --output human feed subscribe ...` → `internal` (not parser error).
- D-009 Template DTO file morphology — clarified `README.md:209` Templates row and `docs/quickstart.md:149` that `--template-json`/`--template-file` accept `WorldTemplateDescriptor`, while `--request-file` must be `CreateWorldFromTemplateRequest` with top-level `{"template": descriptor}`; added both examples and verified `world create --template-file` (descriptor) and `world create --request-file` (request) parse OK while descriptor-as-request correctly fails `missing field template` (validated via `cargo run -p loom-cli --quiet -- ...`).
- D-010 terminalization — fixed `docs/operator-guide.md:53` `dead|dead|Cancelled` pseudo-syntax to `dead` (or `cancelled`) per handler `apps/loom-cli/src/lib.rs:1394-1401`; swept all `terminal-state` examples (`dead` in quickstart/operator) to ensure only `dead`/`cancelled` are used.
