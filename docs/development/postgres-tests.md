...
Agents running commands directly on this development machine should reuse the same local database and invoke PostgreSQL-dependent tests through `bash tools/test.sh ...`. They should not start a second PostgreSQL container per task or invent a different test connection URL.
