...
| **Scheduler** | Deterministic `(effective_due_world_time, logical_schedule_order)` ordering; head-of-line barrier; quiescence; `Chronology Budget` as Timeline Logical State; target-neutral bounded Supervisor discovers existing, new and forked Timelines with `Pending` Work; `docker compose up -d` starts PostgreSQL + `loom-server`; PostgreSQL remains authority and discovery never chooses logical Work ordering; `SKIP LOCKED` only across independent Timeline heads; single-thread executor per worker process, multi-process via shared PostgreSQL | Exact numeric budget/retry defaults as invariants; multi-threaded shared-process Runtime topology |
...
# The running loom-server automatically discovers Pending Work on this and
# later-created or forked Timelines; no target registration or restart is needed.
```

Complete public workflow (see [`docs/quickstart.md`](docs/quickstart.md) for every command with expected output):

`start stack` → `construct WorldTemplateDescriptor + create World` (via `examples/neutral-v0/templates/revision-1.json`) → `invoke Action` (including `neutral.link.create`/`neutral.blob.attach`) → `inspect State/History/Catalog` → `submit Ingress` → `tail/resume feed` (`--after`/`--limit` / `resume_from`) → `automatic bounded Scheduler discovery and Runtime-driven Work/World Time progress` → `replay/fork` → `inspect Runtime Revision/Session provenance` → `deterministic Agency Wake via neutral fixture` → `restart and resume`.
...
- **Server:** `apps/loom-server` — production-like composition root (healthcheck → migrate → validate registry → activate Revision → construct Runtime/Boundary and lifecycle-managed Scheduler Supervisor → bind `LOOM_BIND_ADDR`).
