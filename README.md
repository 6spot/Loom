...
**How to start:** Follow [`docs/quickstart.md`](docs/quickstart.md) — start the stack, construct a `WorldTemplateDescriptor` JSON and create a World, invoke Actions, inspect State/History/Catalog, submit Ingress, tail/resume the Change Feed, configure the Scheduler for the target World/Timeline and let it progress Work/World Time after restart, replay/fork, inspect Runtime Revision/Session provenance and (deferred) run a deterministic Agency Wake when a future public fixture is available. No Runtime/Storage imports or direct database access are required; the public surfaces are `loom-server` HTTP/SSE, `loom-client` and `loom-cli`. Template discovery via `CatalogService` and a ready Relationship/Agency fixture are **deferred** to `M12-T3` (see Supported vs deferred matrix).

## Supported vs deferred — V0 scope matrix

| Domain | V0 supported | V0 deferred / unproven |
| --- | --- | --- |
| **World model** | Create/Load World via caller-constructed `WorldTemplateDescriptor` (TemplateId + `TemplateCapabilityRequirement[]` semver + `initial_world_time`) → Runtime-validated `ValidatedWorldBirthPlan`; immutable `World Runtime Binding`; per-Timeline `TimelineVersion`, `World Time`, `Timeline Logical State` | Dynamic per-World Capability migration / hot-plug; generic `Event Scope`; Template discovery via `CatalogService` (no `CatalogSnapshot` Template field at head); multiple Template revisions proving future-World-only changes and installed-but-disabled semantics (pending `M12-T3` public fixture) |
...
| **Agency** | Scheduler-managed durable `Agency Wake` → `AgentWorldView` (visibility-limited, Binding-checked) → `CognitiveExecutor` → `Decision::Act|NoAction` → normal Capability authority; explicit `resample` vs `ReuseDeterministic` CAS policy (provenance-visible, measured in `loom-bench`) | Deterministic `DeterministicCognitiveExecutor` as default public composition (currently `crates/loom-agency/src/testing.rs`, not `capabilities/loom-neutral`; `loom-server` defaults to `UnavailableCognitiveExecutor` — needs future public fixture/adapter); Real vendor LLM integration as required V0 path; `CognitiveExecutor` inside `WorkHandler` |
...
Local examples use only public surfaces; real vendor LLM credentials are never required for V0. Deterministic fake cognition is used only as benchmark/test evidence (`crates/loom-bench`, `crates/loom-agency/src/testing.rs`); the default `loom-server` public composition uses `UnavailableCognitiveExecutor` — a supported public deterministic fake fixture/adapter is **deferred** to `M12-T3`/T4. Provider credentials belong only to a reviewed provider adapter's application config, not this composition root.
...
`start stack` → `construct WorldTemplateDescriptor + create World` → `invoke Action` → `inspect State/History/Catalog` → `submit Ingress` → `tail/resume feed` (`--after`/`--limit`) → `configure Scheduler (LOOM_SCHEDULER_WORLD_ID/TIMELINE_ID + restart) → progress Work/World Time` → `replay/fork` → `inspect Runtime Revision/Session provenance` → `(deferred) deterministic Agency Wake` → `restart and resume`.
...
- **Neutral examples:** `capabilities/loom-neutral` — `neutral.counter` and `neutral.observer` covering Entity/Facet, Action/Event, cross-Capability dependency (`observer` → `counter`), `Reaction`/`Work` via the installed registry (see `capabilities/loom-neutral/src/lib.rs`). Relationship, multiple Template revisions and `DeterministicCognitiveExecutor` as default public fixtures are **deferred** to `M12-T3` (`crates/loom-agency/src/testing.rs` is test-only; `loom-server` defaults to `UnavailableCognitiveExecutor`). Existing counter/observer examples survive restart/replay/fork and need no vendor secrets.
