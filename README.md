...
| Templates | Caller-constructed `WorldTemplateDescriptor` JSON (`id`, `revision`, `capabilities[]`, `configuration`, `initial_world_time`) submitted to `world create` (`--template-json`/`--template-file`/`--request-file`) and validated Runtime-side into `ValidatedWorldBirthPlan`; World `Binding` is immutable after birth and not retained by the Template (no `CatalogSnapshot` Template discovery at head — see `crates/loom-api/src/lib.rs:1913`) | Multiple Template revisions proving future-World-only changes / installed-but-disabled semantics; Relationship fixture; deterministic Agency fixture (all pending `M12-T3` public fixtures) |
